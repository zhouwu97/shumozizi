...
description: 论文初稿完成后，按数据特征补充类型丰富、风格统一的高级科研图表。适用于整篇论文的视觉增强，不改动数据、模型与结论；核心问题 1 张 Hero 图 + 按需 supporting 图，推荐全篇 13–18 图作为视觉充实度参考信号。
---
...
- **Hero Figure**：每个核心问题通常需要 1 张高辨识度、能被读者记住的 Hero 图（展示整体解空间、渗流网络、帕累托前沿或核心机制）。
- **Supporting Figures**：若一个图不能同时解释数据直觉、机理、权衡或边界，再补充辅助图。
- **全篇视觉节奏**：对于四五问复杂建模赛题，全篇推荐 13–18 张科研图作为视觉证据充足的参考信号，按论证角色与数据特征有机分布，不设生硬均摊指标。
