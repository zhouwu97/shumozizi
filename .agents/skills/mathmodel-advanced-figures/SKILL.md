...
description: 论文初稿完成后，按数据特征补充数量充足、类型丰富、风格统一的解释型图表。适用于整篇论文的视觉增强，不改动数据、模型与结论；每题 2--3 张、全篇至少 12 张是硬规格。
---
...
论文初稿写完、production 结果冻结之后，**必须**执行本 skill，直到正式发布入口满足
每个必答问题 2--3 张 current 正文图、全篇至少 12 张 current 正文图、至少 3 种可审计图型。配额是竞赛交付
规格，不是页数或字数的替代品；每张图仍须承担决定性证据、机制或边界中的明确角色。
...
- 每个必答问题在**正式发布入口**实际消费 2--3 张 current 正文图；全篇实际消费不少于 12 张 current 正文图。重复插入同一图、长稿引用、素材池、未晋级草图和附录图均不计数。
- 已消费图至少覆盖 3 种可审计图型（`visual_archetype` / `advanced_template`）；图型必须由数据结构和论证角色决定，不能用换配色、拆分同一图或装饰图伪造多样性。
