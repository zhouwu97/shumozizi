...
### 1. 先写图合同（claim-first），再挑图种

对每张计划中的正式图，先读 [contract.md](references/contract.md) 并逐条完成五点合同：
**核心结论一句话 → 证据链逐面板 → 图型（archetype）分类 → current 数据来源 →
嵌入与解读契约**。写不出一句话核心结论、或找不到 current 结果字段的图不画。
图合同确认后，才从 [figure-catalog.md](references/figure-catalog.md) 按数据特征挑图种
（不要从图库名称反推数据）。每张图必须能回答：它要解释什么、读者一眼看见什么、
删除后论文失去什么。
...
- 每张正式图在画前完成了五点图合同（核心结论/证据链/图型/数据来源/嵌入契约）；
  缺一句可辩护的核心结论或 current 数据来源的图不算有效论证图。
