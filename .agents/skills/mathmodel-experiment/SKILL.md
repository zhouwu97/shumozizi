...
python scripts/runtime/run_simple_experiment.py runs/<run-id> `
  ...
  --input problem/attachments/data.xlsx --metric objective=123.45
```

执行器使用 `shell=False`，记录命令、开始与结束时间、退出码、stdout、stderr、源脚本、输入输出路径和 SHA-256。只有 `results/index.json` 中 `status=current` 且 `paper_allowed=true` 的条目可以进入论文；新结果会把同问同类型旧 current 结果标为 `superseded`。
