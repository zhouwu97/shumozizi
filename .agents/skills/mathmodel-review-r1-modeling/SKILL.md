...
`remediation`、`status`、`change_class`、`route_impact` 和
`changed_route_core_fields`。证据不得使用聊天记忆或未声明文件。
...
`PROBLEM_INTERPRETATION_CHANGE`。只有后两类或 `route_impact=material` 才属于路线漂移；
`affected_stage=R1_MODELING` 本身绝不代表需要重新批准路线。
