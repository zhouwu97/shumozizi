...
按 request 的唯一 `output_path` 写 `review_report.json`。报告返回生产主对话后，由主 AI 写入
`REVIEW_ADJUDICATION.json`，再生成 `review_receipt.json`；报告 verdict 只能是 `ACCEPT`、`ACCEPT_WITH_MINOR_FIXES`、
