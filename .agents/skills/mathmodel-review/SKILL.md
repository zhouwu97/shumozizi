...
- `diff_check`：只检查声明的局部差异与修复证据，不使无关 R1/R2 回执失效。
...
每轮写入 `review/<stage>/<round>/review_request.json`、`review_report.json`、`REVIEW_ADJUDICATION.json` 和 `review_receipt.json`，并由 `StateService.record_review_gate()` 登记到 `state.json.review_gates`。审核 AI 只写 session 和报告；裁决、修复计划和回执属于生产主对话。
