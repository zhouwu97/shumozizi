...
R1 建模、R2 实验复现、R3 论文逻辑、R4 格式视觉和 R5 全面盲审均使用独立请求、报告和回执。审核任务默认只读；模型、数字、核心图表、假设、约束或结论变化时，按影响范围重跑实验并新开 R5。
...
R1-R4 各执行一次，R5 竞赛模式最多 3 轮、训练模式最多 5 轮。单轮 P0/P1 或评级低于 B 必须修复；连续两轮 B/A 且无 P0/P1 才能进入最终核包。
...
每轮写入 `review/<stage>/<round>/review_request.json`、`review_report.json` 和 `review_receipt.json`，并保留最终汇总报告。
