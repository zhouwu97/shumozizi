...
所有审核先自由诊断，再映射清单和 finding。Reviewer 首先回答：真正解决了什么、是否回答原题、
最薄弱主张、最可能反例、最缺乏信息量的结果、评委不接受的原因和最值得做的一项修复。Schema
只记录判断，不得成为阅读顺序。

## 审核预算

R1-R4 按受影响范围执行。R5 不设固定三轮或五轮上限：只有首次完整候选、核心模型/数据/数字/
结论/主图变化或 P0/P1 重新打开时执行完整盲审，总时间建议占比赛预算 5%–10%。局部修改只做
scoped recheck，且不得生成完整竞赛评分。单轮 A/B 且无 P0/P1 即可通过；预算不足时停在人工
提交或终止决定。J0 最多执行一次，不属于 R5 循环且不是硬门。

机械错误可以直接小修；任何路线漂移必须写 `review/ROUTE_DRIFT_MEMO.md` 并返回人工路线确认。

只有四类硬门可以直接阻断：H1 核心数值/引用/执行真实性失效；H2 未回答 required output 或
目标偷换；H3 数据泄漏、硬约束/单位错误、正控制无检出能力、关键参数不可辨识却输出确定结论、
核心比较不公平；H4 提交无法打开、严重裁切、匿名/官方格式违规或缺必交文件。其他问题按 P1-P3
表达质量影响，不得因为形式字段不完整自动阻断研究。
...
Reviewer 若缺少判断当前问题所必需的原始数据摘要、中间结果、正控制、baseline、目标/约束说明
或图表源数据，可以请求生产主对话用 `freeze_supplemental_evidence()` 创建单问题证据包。补充材料
必须绑定 `supplemental_evidence_manifest`、逐文件动态 role 和哈希，只对本轮问题开放；仍禁止
历史审核结论、作者辩解和未经冻结的临时说明。
