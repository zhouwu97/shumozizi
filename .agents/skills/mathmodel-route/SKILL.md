...
4. 如需领域规范，读取 `skills/_references/math_modeling_norms.md` 的相关小节。
...
1. 按 `schemas/route_candidates.schema.json` 写入 `brief/route_candidates.json`。
2. 写入人工可读的 `brief/ROUTE_BRIEF.md`，内容与 JSON 一致。
3. 更新 `state.json`：
   - `status`: `WAITING_HUMAN_ROUTE`
   - `completed_stages`: 加入 `route_proposal`
   - `active_stage`: `human_route_review`
   - `route_locked`: `false`
4. 执行 `python scripts/codex/validate_state.py runs/<run_id>`。
5. 向用户总结候选路线、推荐理由和需要确认的歧义，然后停止。

不得替用户写入已批准的 `ROUTE_LOCK.yaml`，也不得继续调用分析、实验或论文 Skill。
