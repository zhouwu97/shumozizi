...
初始化 v3.2 运行后，先执行 `python scripts/simple/delivery_control.py status <run_dir>`，并在每次阶段切换、开始新一段工作或准备扩展协议前重查。每段实际工作用 `start-work <category>` 开始、用 `stop-work --summary <产出>` 结束；状态中的活动会话、墙钟覆盖率、超过 20 分钟的未解释缺口和超过 120 分钟的未关闭会话必须如实处理。协议和执行器修复明确归到对应类别，不能漏记或用实验耗时掩盖。
...
5. 图表与洞察嵌入实验和论文：图在 `figures/current/` 并声明 role（稳定性图入附录），核心问题的规律带 `insight_id` 登记，可读版本写入 `INSIGHTS.md`。每个核心问题还必须在 `FIGURE_PLAN.json` 2.1 中显式选择 `required` 或 `waived`；前者至少绑定一张非 stability 正文主图，后者写具体理由，不靠省略计划静默零图。
