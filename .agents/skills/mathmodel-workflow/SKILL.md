...
| `WAITING_HUMAN_ROUTE` | 只检查人工填写的 `brief/ROUTE_LOCK.yaml`；未批准则停止 | `ROUTE_LOCKED` |
...
随后停止。不得自动替人工选择路线，不得生成 `ROUTE_LOCK.yaml` 的批准内容。

用户确认后，检查路线锁至少包含：题意解释、主路线、备用路线、基线、创新、验证和预算。
只有 `approved: true` 时才能把状态改为 `ROUTE_LOCKED`。
