...
| `ROUTE_LOCKED` | 生成数学规格并冻结最低科学合同 | `MODEL_SPEC_READY` |
| `MODEL_SPEC_READY` | 先登记通过的 `R1_MODELING` 回执，再开始正式实验 | `EXPERIMENTING` |
| `EXPERIMENTING` | 对每问运行最小实验、Scientific Viability Check 和 R2；风险路线不进入论文 | `RESULTS_ACCEPTED` 或返回路线竞争 |
| `RESULTS_ACCEPTED` | 仅对 `VIABLE` 或已修复的 `WEAK_BUT_REPAIRABLE` 结果用 `$mathmodel-paper` 组装全文 | `PAPER_DRAFTED` |
...
1. 完整读取 `skills/2analysis-modeling/SKILL.md`，只采用已锁路线，生成可编码的数学规格；同时创建
   `analysis/MINIMUM_SCIENTIFIC_CONTRACT.md`，冻结 required outputs、目标、硬约束、baseline、
   primary 模型族、数据划分、指标、positive control、失败判据、fallback 条件和实验预算。
2. 逐子问题调用 `$mathmodel-experiment`。先实现 baseline，再识别最可能使整篇论文失败的关键
   不确定性，运行最低成本证伪实验；不要并行启动多个 Agent。
3. 最小实验完成后创建或更新 `analysis/SCIENTIFIC_VIABILITY.md`。每次只围绕一个当前最高风险，
   写明反例、最低成本证伪实验、真实结果、baseline/fallback 比较、决策理由和剩余预算，不得做成
   四项打勾表；`ROUTE_FAILED` 还必须声明 failure_origin。
   `VIABLE` 继续深化；`WEAK_BUT_REPAIRABLE` 定向修复；`ROUTE_AT_RISK` 并行最小 fallback 且
   禁止写正式章节；`ROUTE_FAILED` 停止 primary，保留失败证据并返回路线竞争。路线中心变化仍
   必须经过人工路线确认。
4. 每个子问题通过 viability 后创建 R2 请求并停在独立对话交接点；审核 AI 自动执行并返回报告，
   主 AI 独立裁决 finding、登记有效回执，再调用 `$mathmodel-paper` 写对应章节。
5. 仅当论文需要解释方法链路时，完整读取并使用 `skills/4drawio/SKILL.md`。
6. 全部子问题完成后用 `$mathmodel-paper` 汇总摘要、结论、优缺点和推广。
7. 依次为 R3、R4 和 R5 创建独立审核请求；每次都由用户新开对话、其中的 AI 自动审核，主 AI
   返回生产对话后裁决，再执行机械 QA 或后续阶段。只有所需回执登记且绑定仍有效时，运行到
   `WAITING_HUMAN_FINAL` 并停止。

J0 可在 R5 后作为自然评委模拟执行，但不是进入人工终审或 `COMPLETE` 的必要条件。

Scientific Viability 不写入新状态枚举，也不替代真实性与 seal。运行
`python scripts/codex/scientific_viability.py verify runs/<run_id> --paper-entry` 必须通过后，结果才可
进入正式全文。阈值只能来自当前题目的容许误差、工程尺度、决策边界、baseline、搜索域或领域
合理范围，不得跨题套用统一阈值。
