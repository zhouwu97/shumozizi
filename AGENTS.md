...
- Author Pass 与长篇首稿在后台维护 `paper/generated/VISUAL_REQUIREMENTS.json`，按数学对象、决定性证据、机制和边界检查 current 图覆盖。视觉设计以 SCI-Box 科研图母版为默认来源（支持 direct / adapted / remix）：核心问题通常配备 1 张高辨识度 Hero Figure，必要时补充 supporting figures；全篇推荐 13–18 张科研图作为多问复杂赛题视觉充足的参考信号，按论证角色与数据特征有机分布，不设机械均摊阻断门。正文中每张图必须完成“观察（显示了什么）→ 机制（为什么呈现该形态）→ 结论（对模型与答案意味着什么）”的完整三步论证。
