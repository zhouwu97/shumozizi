...
- `optimization`、`simulation`、`exact_oracle` 和 `coordination` 单元必须填写 `capability_decision` 并显式考虑 MATLAB/Octave。MATLAB 可以因真实环境不可用、解析路线更直接、不能形成异构算法族或 Python 已有精确证书而不入选，但不得静默忽略；选用时必须声明 `primary_model`、`optimizer_challenger`、`independent_oracle` 或 `scientific_visualization` 角色。MATLAB 图像统一先写入 `figures/work/<figure_id>/<version>/`。
- 任何“分别求解再组合”的路线必须声明为精确分解、启发式分解或仅作联合优化初值。精确分解给出等价依据；启发式/初值路线必须继续用联合 scorer 改进。没有证明、小规模对照或联合后续时，不得把各子问题分别最优写成全局最优。
- 优化比较统一 exact 目标、实际预算与可行性优先；首个可行解后至少执行一类计划内深化，第二类只在仍有决策价值时增加，停止理由必须与搜索日志一致。灵敏度、鲁棒性和题型 oracle 条件触发，不能互相替代。
...
- 候选稿前运行 `scripts/paper/audit_report_style.py`，识别多问重复小节、内部工作流词、“本问采用/结果见表”式报账、摘要逐问罗列、列表堆叠、标题碎片化、核心问题缺推导/机制及主图脱离论证。检测结果只作 warning 并进入机械 QA 与盲评问题，不新增 Schema 或状态硬门；自动信号不能替代独立 PDF 阅读。
