...
先做不变量/上下界、重参数化、分解、事件、小规模 oracle 与可辨识性风险的结构预检；再按预检论证读取最多三个能力包（主能力、交叉能力、验证/不确定性）。知识只提出候选，先生成两到三条实质不同的候选路线，再做最低成本 probe 并确定主路线与 fallback。实现错误直接修复；参数或求解器问题在路线内调整；fallback 更优时直接切换并记录。
...
它生成 `qa/mechanical-qa.json`、`qa/contact-sheet.png` 与 `reports/VERIFY_REPORT.md`，并检查 PDF、路径、占位符、匿名、图表可读性、current 结果哈希、指标来源、结果引用与数值一致性等确定性问题。匿名投稿必须显式追加 `--anonymous`，必要时用 `--anonymous-term` 指定禁止身份词；图表/表格编号目前只作 warning。最终整体审查输出 `qa/FINAL_REVIEW.md`，按 P0–P3 定位问题；除重大模型、数据或结论修改外，不重复整篇审查。
