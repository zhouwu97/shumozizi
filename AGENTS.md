...
自动发现目录 `.agents/skills/` 保留以下十项主动能力：
...
用户只要求数据分析、调试代码或修改论文时，不得自动启动完整工作流。完整赛题由连续生产、独立科学红队、论文、独立 PDF 盲审和机械终检组成；两个审查必须各自新建 Codex 对话，不能复用求解或论文上下文。它们不是 legacy-v2 的按问审核平台：每个完整 run 只在论文前和 PDF 后各有一次明确的独立边界。
