...
严格编号升序。完整 R5 只由核心模型、数据/数字、结论、主要图表变化或 P0/P1 重新打开触发，
总时间建议控制在比赛预算的 5%–10%；无核心变化时禁止重复完整 R5，局部修改只做 scoped
recheck。J0 为可选评委模拟。每一轮审核必须由用户在 Codex 桌面版
...
必须区分 `raw_score` 与受硬条件封顶的 `calibrated_score`。scoped R5 不生成完整竞赛评分，
也不能冒充完整盲评；核心变化但时间预算不足时停在人工提交或终止决定。
