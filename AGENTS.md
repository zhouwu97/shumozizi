...
- 前期可用 fresh thread、获奖论文结构卡或网页 GPT 讨论题意、建模、反例、验证和论文建议；专家卡和网页讨论都是发现问题的可选手段，不是阶段门。进入实验前的正式 `MODELING_UNITS.json` 使用 1.4：所有问题先完成一次 `faithful_reconstruction`，忠实重建决策变量、成功事件、聚合和输出。语义高风险不能只由执行者自报：系统还会从 `coordination` 单元、资源间聚合文本、非联合分解路线、待比较 endpoint 和显式嵌套量词派生风险；任一派生风险、`question_delta.must_recheck_aggregation=true` 或 endpoint 仍为 `comparison_planned` 时，增加一次全局 `semantic_adversary`，并要求**每个**高风险单元各自预登记最小反例、在正式路线搜索前真实执行 3--5 个 scorer 语义案例，其中至少一个明确绑定该反例。全局 adversary 可以复用，逐单元 production 反例不能复用其他问题的结果。报告中的语义结论是科学硬门；thread ID、task receipt 与 `problem/` 树哈希只作独立性记录，缺少这些元数据不得否决低风险题。回执与多份报告的一致只证明独立上下文和共识，绝不证明目标正确。应及时把改变路线的判断写成绑定 `problem/` 的 `analysis/BASELINE_FREEZE.json` 决策快照，并在问题、反例或实验冲突出现后修订、重跑和复审。
...
- 每个必答问题在路线比较前必须在 `MODELING_UNITS.json` 写 `question_delta` 和直接答案合同：先比较相邻问题新增的实体、资源、共享约束与聚合层，再声明要求输出、决策对象/总体、主 endpoint/estimand、主判据、自然 baseline 和 fallback。主 endpoint 必须同时用自然语言与公式写清原子成功、实体内、资源间、实体间、时间和量词次序；聚合口径有歧义时标记 `comparison_planned`，先用玩具反例区分，再对仍合法解释跑后果 probe，不能看完结果后换 endpoint。每个系统或人工识别的高风险单元（不只 `core_question`）都必须在路线搜索前真实执行 3--5 个人工 scorer 案例，并逐案例保存预期排序、实际排序和通过状态。
- `MODELING_UNITS` 1.4 按题型使用 `evaluation`、`optimization`、`exact_oracle`、`data_modeling`、`simulation` 或 `coordination`。固定评价、数据建模和仿真写主方法与自然核对，不强迫赛马；`exact_oracle` 同时比较正式指标容差和区间/集合结构。普通题使用主方法加自然比较；核心优化/协同题使用 baseline 加一条数学结构不同的 challenger，只有首轮仍不确定、仍在改善或评审明确指出缺口时才增加第二条 challenger。单纯替换 GA、PSO、DE 属于同一数学路线。每个单元显式声明 `core_question`，且运行至少有一个核心问题。新质量合同中的 `data_modeling` 还须在 `data_contract.outcome_kind` 明确写 `recommendation` 或 `descriptive`；前者一律要求 `recommendation_uncertainty.required=true`、具体方法和 production 不确定性结果，后者才允许 `false` 并说明没有推荐对象的理由。
...
- Author 默认只接收三个概念：`paper/author-pass/RESEARCH_PACKAGE.md`、`paper/author-pass/AUTHOR_BRIEF.md` 和冷读/Editorial Pass 的高价值编辑反馈；`PAPER_BLUEPRINT.md`、`paper/answer-map.json`、素材池、故事板、`FIGURE_PLAN`、claim gate 与 generated JSON 只作后台兼容、事实投影和最终审计，不得成为创作前置清单。Research Package 必须为 Writer 提供完整的科学原料（数学对象定义、关键推导、约束物理意义、模型选择理由、求解算法适配依据、Baseline/Challenger 对比、代表性反例、机制发现、敏感性边界与图解读），严禁进行任何机械数量截断；按科学功能语义去重。联网国赛在此之前须完成一次双语检索、候选核验与 citation ledger，零命中也如实记录。Author Pass 只以正式 objective answer、current production 绑定和已关闭 scientific P0/P1 为科学硬门。
- 正文的核心是模型建立、数学推导、求解方法与结果机制分析（建议篇幅占比 50–60%）：必须解释模型选择理由（与更简单 baseline 对比）、变量与参数定量依据、关键数学推导展开、约束物理意义以及为什么结果呈现当前结构。正文必须使用连续学术段落，列表仅用于必要假设、符号说明、极简算法步骤与优缺点摘要。语言采用自然、正式、克制的中文学术表达，允许使用“本文建立”、“由式(x)可得”、“这说明”、“其原因在于”等；正文禁止使用“我们/大家”等第一人称，杜绝工作汇报式报账。
- Author Pass 与长篇首稿在后台维护 `paper/generated/VISUAL_REQUIREMENTS.json`，按数学对象、决定性证据、机制和边界检查 current 图覆盖。视觉设计以 SCI-Box 科研图母版为默认来源（支持 direct / adapted / remix）：每个必答问题必须在正文消费 **2–3 张**承担不同论证角色的 current 正文图【硬门】；四问及以上正式稿全篇消费 **13–18 张** current 正文图【硬门】且覆盖至少 **3 种**可审计 visual archetype【硬门】；少于四问按论证角色编辑复核。数量要求不得通过重复图、拆分同一图或装饰图凑数。核心问题配备 1 张高辨识度 Hero Figure，必要时补充 supporting figures，并配备全局/跨问模型路线或数据结构图。正文中每张图必须完成“观察（显示了什么）→ 机制（为什么呈现该形态）→ 结论（对模型与答案意味着什么）”的完整三步论证。
- 候选稿前运行 `scripts/paper/audit_report_style.py`。只有 E001 正文泄漏内部术语属于确定性硬错误；E002--E005（重复报账模板、无统一主线的逐问摘要、核心问过度列表化、图后论证薄弱）与句长、标题、列表密度和篇幅均为 editorial signal，由独立 PDF 阅读裁决，不能直接阻断或诱导 Author 按检查项补句。
- 完整长稿 PDF 编译后引入只看成品的 Editorial Pass：直接审阅页面排版密度、空白度、小节碎片化、数学推导展开深度、机制解释与图文结合度，直接针对论文源码进行优化修改，不新增繁琐流程回执与 Schema 阻塞。
