...
python scripts/runtime/run_simple_experiment.py runs/2026-A-001 `
  ...
  --metric objective=123.45
```

执行器固定 `shell=False`，记录命令、退出码、stdout/stderr、源脚本、输入输出哈希和执行时间。新结果会将同问同类型旧结果标记为 `superseded`；只有仍为 `current` 且 `paper_allowed=true` 的结果才能进入论文。
...
```powershell
python scripts/qa/run_final_checks.py runs/2026-A-001
```

该命令生成：

- `qa/mechanical-qa.json`：PDF、空白页、裁切、文字重叠、重复编号、占位符、失效结果引用和显式关键数字检查；
- `qa/contact-sheet.png`：便于人工快速查看的 PDF 联系表；
- `reports/VERIFY_REPORT.md`：简短可定位的验证摘要。

一次整体审查写入 `qa/FINAL_REVIEW.md`，只判断题目覆盖、逐问回答、模型目标一致性、结果信息量、baseline/正控制、公平性、可辨识性和结论边界。P0/P1 可触发一次定向修复；P2/P3 和排版小改不重新开启完整审查。
