...
R1-R5 的每轮审核都由用户在 Codex 桌面版中新开一个独立顶层对话。用户只负责完成这一步对话
交接；新对话中的 AI 会自动领取请求、读取冻结材料、执行审核并写入报告，用户无需辅助
检查、复现、测量、判分或填写报告。审核对话不得使用子 Agent、fork 或生产聊天上下文，也不得
修改生产产物。报告返回生产主对话后，由主 AI 独立核验 finding，写入
`REVIEW_ADJUDICATION.json`，再生成回执和后续修复计划。R4/R5 的 `FORMAT_AUDIT.json` 机器
硬失败不可由文字结论覆盖；R5 正式竞赛判断使用封顶后的 `calibrated_score`。
