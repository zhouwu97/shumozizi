...
> **v3.4 科学优先修订（高于本文旧数量建议）**：新运行使用单主线与科学检查点。图数、Hero 图数量、前五页位置、页数和章节顺序均为编辑信号，不得成为硬门；只有语义、生产事实和发布事实会阻断流程。旧 `competition-quality-v1` 运行才继续适用本文中的数量兼容规则。
...
1. **Hero 图**：按中心论点需要选择少数高记忆点主图；前五页是否出现由读者理解测试决定。
...
- 前五页应尽早出现能帮助理解中心论点的图；是否需要高级图由冷读结果决定。
