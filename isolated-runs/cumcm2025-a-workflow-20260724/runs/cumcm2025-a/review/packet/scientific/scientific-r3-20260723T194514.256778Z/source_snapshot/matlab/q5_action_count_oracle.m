...
q3 = jsondecode(fileread(fullfile(runRoot, 'results', 'raw', 'q3.json')));

actions14 = q5.result.actions(:);
extraAction = q3.result.actions(3);
extraAction.assigned_missile = 'M3';
actions15 = [actions14; extraAction];
assert(numel(actions14) == 14 && numel(actions15) == 15, '动作数输入不完整');
...
for index = 1:numel(actionCounts)
    ...
    coarseScores(index, :) = scoreActions(actions15(1:count), missileNames, ...
        missileInitial, targetPoints, coarseStep);
    fineScores(index, :) = scoreActions(actions15(1:count), missileNames, ...
        missileInitial, targetPoints, fineStep);
end

[coverage202505, coverage202506] = coverageSeries(q5.action_count_coverage);
pythonObjectives = [coverage202506(14).objective_missile_s; ...
    coverage202506(15).objective_missile_s; coverage202506(16).objective_missile_s];
maxProductionError = max(abs(fineScores(:, 1) - pythonObjectives));
maxStepDifference = max(abs(fineScores(:, 1) - coarseScores(:, 1)));
assert(abs(fineScores(3, 1) - fineScores(2, 1)) <= 1e-8, ...
    'MATLAB 独立重算未确认第 15 动作无可辨识增益');
