...
def _coverage(question: str, pool: dict[str, object]) -> dict[str, object]:
    ...
    if question == "Q5":
        ...
        return {"group_reports": [{"id": "all_action_counts", "variables": ["action_count"], "metric": "occupied_bins", "occupied_cells": len(values), "possible_cells": 16, "joint_coverage": len(values) / 16.0}]}
...
def main() -> int:
    ...
    if question == "Q5":
        ...
        if not coverage_ok or len(records) < 2:
            challenge = "model_or_scorer_semantic_error"
    payload = {"schema_name": "search_audit", "adapter_id": "cumcm2025a-independent-quality", "adapter_version": "1.0", "candidate_count": len(pool["candidates"]), "exact_candidate_count": len(exact["candidate_scores"]), "coverage": _coverage(question, pool), "calibration": _calibration(pool, exact), "challenge": {"outcome": challenge}}
