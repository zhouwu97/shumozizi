...
RESULT_IDS = {"Q1": "q1_quality_verified", "Q2": "q2_quality_verified", "Q3": "q3_quality_verified", "Q4": "q4_quality_verified", "Q5": "q5_quality_verified"}
...
def _contract(question: str) -> dict[str, object]:
    ...
    return {
        ...
        "adapter_version": "1.0",
        "selection_contract": {"objective": {"metric": metric, "direction": "maximize", "objective_version": "finite-cylinder-union-v1", "scorer_version": "event-brent-v1", "constraint_version": "flight-kinematics-v1", "semantics": "union", "fine_tolerance": 1e-8}, "coverage": {"candidate_variables": [variable], "groups": [{"id": group, "variables": [variable], "metric": "occupied_bins", "bins_per_variable": bins, "bounds": {variable: [lower, upper]}, "minimum_joint_coverage": 1.0}]}, "required_evidence": ["finite-cylinder-exact-score", "continuous-geometry-certificate"], "required_prior_questions": [] if question == "Q1" else [f"Q{int(question[1]) - 1}"]},
        "stages": {
