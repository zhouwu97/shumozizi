...
**本阶段不可整体跳过。** 经验教训：跳过本阶段会导致正文出现"考虑图1的三层结构"
"多光束干涉（图2）"这类**指向不存在的示意图**的悬空引用——评审一眼可见的硬伤。
两条硬性要求：

1. **至少产出一张技术路线图**（`fig_roadmap`）。
2. **物理/结构/机理类题目必须产出对应的原理或结构示意图**（如光路图、三层结构图、
   受力图、系统结构图）。凡是正文要"配合示意图讲清机理"的地方，都必须真有这张图。

**严禁在正文写死"图1""图2"这类手工编号来指代示意图。** 图号必须由论文阶段用
交叉引用生成（LaTeX 的 `图~\ref{fig:...}`、Typst 的 `@fig-...`），否则图表一重排就
错位。`6verity` 的门禁会把写死的 `图N/表N/式N` 判为 FAIL。
