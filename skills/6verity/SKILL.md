...
除结构/引用/占位符等形式检查外，脚本还包含以下**实质性硬检查**（针对历史上"形式通过、实质不合格"的漏网硬伤）：

- **写死的图/表/式编号**：正文出现 `图1`/`表 2`/`式(3)` 等手工编号 → FAIL。必须改用
  交叉引用（LaTeX `图~\ref{fig:x}`、Typst `@fig-x`），否则图表重排即错位，且易出现
  "正文说图1是示意图、实际图1是另一张图"的语义错位。（附件1/附件2 等赛题数据文件名不受影响。）
- **中文论文的英文图**：对每张被正文引用的图 PDF 用 `pdftotext` 提取文字，若提取出
  大量长英文词（坐标轴/图例为英文）而论文为中文 → FAIL。根因修复见
  `3coding-visual/scripts/mpl_setup.py`。此检查需要 `pdftotext`（poppler-utils），缺失则跳过。
- **作图源码未配中文字体**（WARN）：中文论文的 matplotlib 脚本未引入 `mpl_setup`/未设 CJK 字体。
- **缺技术路线图/示意图**（WARN）：`figures/` 与正文引用中找不到 roadmap/流程/结构/示意类图，
  提示 `4drawio` 阶段可能被跳过。
- **缺模型评价/推广章节**（WARN）：中文论文未见"模型评价/优缺点/推广/误差分析"相关章节。

WARN 不阻断，但应尽量清零以贴近国奖水准；FAIL 必须修复。
