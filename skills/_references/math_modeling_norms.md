...
- **中文论文的数据图必须用中文标签**：matplotlib 需先注册中文字体（见 `3coding-visual/scripts/mpl_setup.py` 的 `apply_chinese_style()`），严禁用默认 DejaVu Sans 产出英文轴名或豆腐块。这是最常见的国奖失分点之一。
- **图号一律用交叉引用，不写死"图1/图2"**：正文用 `图~\ref{...}`（LaTeX）或 `@fig-...`（Typst）指代图表；手写编号会在图表重排时错位，并常导致"正文说图1是示意图、实际图1是别的图"的语义错位。
...
- 硬错误包括：核心正文缺失、入口 include 文件不存在、占位符残留、图片路径不存在、关键数值与结果记录冲突、模板结构被破坏、可用编译器下编译失败、**写死的图/表/式编号引用**、**中文论文出现英文标签的数据图**。
- 警告包括：未引用备用图、章节过短、图表后缺少解释、caption 过长、参考文献偏少、代码完整复现耗时过长、**缺技术路线图/示意图**、**缺模型评价与推广章节**、**作图脚本未配置中文字体**。
