...
def _paper_audit_prompt() -> str:
    """返回只允许依据冻结 PDF 作答的网页版论文审核提示。"""
    return """这是一次数学建模论文的受限最终审核。你只收到一份冻结 PDF；不要联网搜索、浏览、引用或复用本题答案、题解、往届论文、相近题现成结论或任何外部资料。必须只根据 PDF 内部的数学定义、推导、数值、表格、图和版式进行核查。\n\n请全面验证审核论文：评估其相对普通数学建模参赛论文的竞争度、可读性和论证主线；检查目标、假设、模型、推导、算法、结果和结论是否自洽，是否存在论证不充分、错误推断、数字/图表不一致或无法由论文自身支持的答案。检查排版、层级、表格、公式、图注和整体视觉是否清楚、专业且符合大众审美；判断每张图是否真正支撑相应论点，并检查图中是否有遮挡文字、重叠标签、裁切、不可读坐标、图例不清或无关装饰。\n\n由于没有题面、源码、运行记录和外部资料：不能凭记忆判断原题正确答案，也不能宣称已验证全局最优；遇到无法从 PDF 内部验证的内容，必须明确标为“需要本地复算/对照题面”，并给出最低成本验证动作。\n\n请写详细报告：先给总体判断和竞争度定位，再按 P0/P1/P2/P3 列出每个发现的页码或位置、问题、影响、证据、可执行修复方案和修复后验证方法。最后给出优先级排序的局部修复计划。不要要求无必要地整篇重写；除非存在根本性模型或证据失效，否则优先建议最小范围修补。\n"""
