...
def _render_author_brief(
    ...
    lines = [
            ...
            "",
            "高级图配额是硬要求：每个必答问题正文必须配 2--3 张 current 支撑图，正式稿全篇"
            "至少 12 张 current 正文图、至少 3 种可审计图型。图必须分别承担数据画像、模型机制、决定性证据、"
            "权衡/边界或验证中的真实论证角色，不能用同一张图反复插入凑数。优先采用小提琴图、"
            "森林图、帕累托前沿、灵敏度图、混淆矩阵、PR/ROC、校准曲线、误差带或收敛图；"
            "普通折线/柱状图在确实最清楚时同样可用。所有正式图均从 current 数据或正式结构 renderer"
            "生成，保持统一字体、坐标轴/单位/图例，并配图号、图注和一句可复述的论证解释。",
            "",
