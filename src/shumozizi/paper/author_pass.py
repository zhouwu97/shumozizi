...
def _render_author_brief(
    ...
    lines = [
            ...
            "",
            "论文必须是国奖级完整竞赛论文，篇幅由论证充分性决定，不设人为上限："
            "每个问题都必须有完整的模型建立、求解、结果解读与支撑图，正文通常 20 页以上"
            "（不含附录），附录可含核心代码与稳定性图。禁止用'篇幅从简'或'图少'规避论证。",
            "",
            ...
            "（3）模型假设（每条假设配合理性说明）；"
            "（4）符号说明（变量/含义/取值三列）；"
            "（5）模型建立与求解（每个问题单独成节：建模、参数估计、结果、直接答案）；"
            "（6）误差分析与检验（对照验证、灵敏度、残差、统计显著性）；"
            "（7）模型评价与推广（优缺点与推广价值）；"
            "（8）参考文献；（9）附录（核心代码与稳定性证据）。",
            "",
            "文风必须是学术论文而非技术报告："
            ...
            "",
            "每个问题的正文必须配 2--3 张支撑图，全篇不同类型图不少于 12 张，且优先采用高级图"
            "（小提琴图、箱线图、森林图、帕累托前沿、肘图、灵敏度/龙卷风图、混淆矩阵、PR/ROC、"
            "校准曲线、误差带曲线、收敛曲线、分布直方图等），而不是只有折线图和柱状图。"
            "所有图必须用 mathmodel-advanced-figures skill（scripts/render_advanced.py + style.py）"
            "的统一样式从 production 数据渲染，DPI≥300、SimSun 中文字体、坐标轴/单位/图例齐全，"
            "每张配图号、图注与一句'这张图展示了什么、能得出什么结论'。",
            "",
