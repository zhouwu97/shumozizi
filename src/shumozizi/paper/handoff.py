...
from shumozizi.paper.repair_loop import require_no_open_repairs
...
def writer_handoff_readiness(run_dir: Path) -> dict[str, Any]:
    ...
    # Argument：核心问题的论证链由上游 canonical 门禁唯一负责——进入论文前
    # require_v32_experiment_evidence → validate_modeling_units 已强制核心问题
    # 拥有机制类 insight（含非空 mechanism 与 evidence_result_ids）。这里不再
    # 重复判定同一事实，避免 double gate 与规则枚举漂移；Author Pass 只把
    # 上游状态投影为 warrant 候选（见 author_pass._warrant_sources）。
    layers["argument"] = "upstream"

    # Repair：已登记但未通过验收的修复指令必须阻断交接（验收不过不能 close）。
    # 这只约束"记录在案的修复"，不影响从未使用 dispatcher 的运行。
    try:
        require_no_open_repairs(root)
        layers["repair"] = "ok"
    except ContractError as exc:
        reasons.append(f"repair: {exc}")
        layers["repair"] = "blocked"
