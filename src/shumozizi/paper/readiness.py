...
def validate_candidate_visual_assessment(run_dir: Path) -> list[str]:
    ...
    """
    from shumozizi.paper.visual_discovery import validate_visual_discovery_closure
    ...
    discovery_errors = validate_visual_discovery_closure(run_dir)
    if (run_dir / _FIGURE_PLAN_PATH).is_file():
        errors = validate_required_figure_consumption(run_dir)
        return (
            [f"VISUAL_NOT_ASSESSED：{item}" for item in errors]
            + requirement_errors
            + discovery_errors
        )
    return (
        _visual_opportunity_assessment_errors(run_dir)
        + requirement_errors
        + discovery_errors
    )
