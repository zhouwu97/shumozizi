...
class DeterministicPrecheckFinding:
    """由固定文本或结构规则产生的预检 finding。"""
...
def _time_leakage(case: dict[str, Any]) -> DeterministicPrecheckFinding | None:
    ...
    if future_signal and shuffled_split:
        return DeterministicPrecheckFinding("时间泄漏", "R2_EXPERIMENT", "P1", ["未来信息与随机切分同时出现"])
    return None


def _target_leakage(case: dict[str, Any]) -> DeterministicPrecheckFinding | None:
    ...
    if target_stat and global_fit:
        return DeterministicPrecheckFinding("目标派生特征泄漏", "R2_EXPERIMENT", "P1", ["目标统计量在验证切分前构造"])
    return None


def _unit_error(case: dict[str, Any]) -> DeterministicPrecheckFinding | None:
    ...
    if mixed_units and missing_conversion:
        return DeterministicPrecheckFinding("公式量纲错误", "R1_MODELING", "P1", ["kW、s 与元/kWh 未换算即相乘"])
    return None


def _nonidentifiable(case: dict[str, Any]) -> DeterministicPrecheckFinding | None:
    ...
    if product_only and reports_separate:
        return DeterministicPrecheckFinding("参数不可辨识但拟合优度很高", "R1_MODELING", "P1", ["参数仅以乘积 a*b 进入模型"])
    return None


def _wrong_formula(case: dict[str, Any]) -> DeterministicPrecheckFinding | None:
    ...
    if "线性衰减" in mechanism and "distance ** 2" in formula and "两份代码" in shared:
        return DeterministicPrecheckFinding("两份代码共享同一错误公式", "R1_MODELING", "P1", ["线性机制与平方衰减公式矛盾"])
    return None


def _group_split(case: dict[str, Any]) -> DeterministicPrecheckFinding | None:
    ...
    if repeated and random_rows:
        return DeterministicPrecheckFinding("重复测量样本泄漏", "R2_EXPERIMENT", "P1", ["重复测量按行随机拆分"])
    return None


def _hidden_constraint(case: dict[str, Any]) -> DeterministicPrecheckFinding | None:
    ...
    if required_floor and omitted:
        return DeterministicPrecheckFinding("遗漏隐藏硬约束", "R1_MODELING", "P0", ["题面库存下限未进入优化约束"])
    return None


def _weak_robustness(case: dict[str, Any]) -> DeterministicPrecheckFinding | None:
    ...
    if tiny and material_gap:
        return DeterministicPrecheckFinding("稳健性实验无辨识力", "R2_EXPERIMENT", "P2", ["扰动量级远小于结论差距"])
    return None


def _direction_reversal(case: dict[str, Any]) -> DeterministicPrecheckFinding | None:
    data = _text(case, "data_or_fixture", "problem_fragment")
    model = _text(case, "model_spec", "code_or_pseudocode")
    ...
    if values and minimizes and chooses_max and float(values.group(2)) > float(values.group(1)):
        return DeterministicPrecheckFinding("图表数字正确但论文结论方向相反", "R3_PAPER_LOGIC", "P1", ["最小化目标却选择更大成本"])
    return None


DETERMINISTIC_PRECHECK_RULES: tuple[
    Callable[[dict[str, Any]], DeterministicPrecheckFinding | None], ...
] = (
    _time_leakage,
    _target_leakage,
    _unit_error,
...
def evaluate_deterministic_prechecks(cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    rows = []
    for case in cases:
        findings = audit_deterministic_prechecks(case)
