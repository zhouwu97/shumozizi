...
class CapabilityFirstV3Tests(unittest.TestCase):
    ...
    def test_active_capabilities_route_to_modeling_and_figure_assets(self) -> None:
        ...
        self.assertIn("knowledge/model-selection-matrix.md", solve)
    ...
    def test_production_skills_require_recovery_and_quality_evidence(self) -> None:
        ...
        self.assertIn("五项决定", workflow)
        self.assertIn("由运行时生成", workflow)
        self.assertIn("已知可行例", solve)
        self.assertIn("最低成本 probe", solve)
        self.assertIn("题目特定精确评分", experiment)
        self.assertIn("独立搜索审计", experiment)
        self.assertIn("diagnostic", experiment)
        self.assertIn("只写已通过科学审查", paper)
        self.assertIn("默认使用 LaTeX", paper)
        self.assertIn("独立 PDF 盲审", paper)
        self.assertIn("`qualified` 或 `strong`", final_check)
        self.assertIn("`weak`", final_check)
